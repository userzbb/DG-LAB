...
1. **前置条件**
   - 设备固件支持 WiFi 功能
   - DG-LAB 官方 APP 已安装（用于扫码绑定设备）

2. **连接步骤**
   - 在设备扫描页面切换到"WiFi"标签
   - (可选) 输入自定义服务器地址，留空则使用官方服务器 `wss://ws.dungeon-lab.cn`
   - 点击"连接 WiFi 设备"按钮
   - 等待系统生成二维码和连接 URL
   - 使用 DG-LAB 官方 APP 扫描二维码或手动输入 URL
   - 在 APP 中完成设备绑定
   - 绑定成功后会自动跳转到控制页面

3. **取消连接**
   - 在等待绑定期间，点击"取消"按钮可停止连接流程
...
**解决方案**:
- ✓ 确认设备固件支持 WiFi 功能
- ✓ 确保 DG-LAB 官方 APP 已安装
- ✓ 检查服务器地址格式（wss://域名 或 ws://IP:PORT）
- ✓ 确认二维码清晰可见，尝试放大窗口
- ✓ 检查网络连接，确保能访问服务器
- ✓ 如使用自定义服务器，检查防火墙设置
- ✓ 尝试复制 URL 手动输入到 APP 中
